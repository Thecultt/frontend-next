...
import { INITIAL_VALUES, SCHEMA, IConciergeMainApplicationFormValues } from './validate';

const ConciergeMainApplicationForm: React.FC = () => {
    const dispatch = useDispatch();

    const { isSendFormCustomProductSuccess } = useTypedSelector(({ concierge }) => concierge);
    const { user } = useAuthUser();

    const initialValues: IConciergeMainApplicationFormValues = {
        ...INITIAL_VALUES,
        name: user.name ?? '',
        phone: user.phone ?? '',
    };

    const sendForm = (data: IConciergeMainApplicationFormValues) => {
                ...
                <Form className="concierge-application-form" onSubmit={handleSubmit}>
                    ...
                    <div className="concierge-application-form-input">
                        <FormikTextarea
                            ...
                            placeholder="Описание желаемого лота под заказ"
                            name="comment"
                        />
                    </div>
                    ...
                    <div className="concierge-application-form-btn">
                        <Button
                            ...
                            className="concierge-application-form-btn__btn"
                            disabled={isSendFormCustomProductSuccess || !isValid}
                        />
...
export default ConciergeMainApplicationForm;
